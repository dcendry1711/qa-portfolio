...
test.describe("Login test cases - sauceDemo", () => {
  ...
  test("LOGIN-TC01 - Successful login", async ({ loginPage, inventoryPage }) => {
    await loginPage.login(VALID_USERNAME, VALID_PASSWORD);
    await expect(inventoryPage.siteHeader).toContainText("Swag Labs");
  });

  //LOGIN-TC02 - Invalid login
  ...
  test("LOGIN-TC07 - Username with leading/trailing whitespace", async ({ loginPage }) => {
  ...
  test("LOGIN-TC08 - Password with leading/trailing whitespace", async ({ loginPage }) => {
  ...
  test("LOGIN-TC10 - Submit login with Enter key", async ({ page, loginPage, inventoryPage }) => {
  ...
  test("LOGIN-TC11 - Refresh after successful login", async ({ page, loginPage, inventoryPage }) => {
    await loginPage.login(VALID_USERNAME, VALID_PASSWORD);
    await page.reload();
    await expect(page).toHaveURL(URLS.INVENTORY_URL);
    await expect(inventoryPage.siteHeader).toBeVisible();
